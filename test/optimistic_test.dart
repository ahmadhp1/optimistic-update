import 'package:optimistic/src/operation_manager.dart';
...
class TestModel implements OptimisticModel {
  ...
  @override
  String toString() => 'TestModel(id: $id, value: $value)';
}
...
void main() {
  group('OptimisticOperationManager Tests', () {
    ...
    test('initialize() should set the initial state correctly', () async {
      // Arrange
      final initialModels = [
        TestModel(id: '1', value: 'test1'),
        TestModel(id: '2', value: 'test2'),
      ];
      ...
      expect(emittedStates.last, initialModels,
          reason: 'Last emitted state should match initial models');
    });

    test('performOperation() should update local state immediately', () async {
      // Arrange
      final initialModel = TestModel(id: '1', value: 'initial');
      final updatedModel = TestModel(id: '1', value: 'updated');
      ...
      expect(
        emittedStates.last
            .any((model) => model.id == '1' && model.value == 'updated'),
        true,
        reason: 'Local state should be updated immediately with new value',
      );
    });

    test('performOperation() should handle multiple sequential operations',
        () async {
      // Arrange
      ...
      await manager.performOperation(
        ...
        newState: TestModel(id: '1', value: 'v1-updated'),
      );
      await manager.performOperation(
        previousState: model2,
        newState: TestModel(id: '2', value: 'v2-updated'),
      );
      ...
      expect(
        emittedStates.last.every((model) =>
            (model.id == '1' && model.value == 'v1-updated') ||
            (model.id == '2' && model.value == 'v2-updated')),
        true,
    ...
    test('should revert state when operation ultimately fails', () async {
      ...
      await manager.performOperation(
        previousState: initialModel,
        newState: TestModel(id: '1', value: 'updated'),
      );
      ...
      // Assert
      expect(
        emittedStates.last.any((m) => m.id == '1' && m.value == 'initial'),
        true,
        reason:
            'State should be reverted to initial value after permanent failure',
      );
    });
    ...
    test('should handle dependent operations correctly', () async {
      ...
      // Assert
      expect(
        emittedStates.last.every((m) =>
            (m.id == '1' && m.value == 'v1') ||
            (m.id == '2' && m.value == 'v2')),
        true,
        reason:
            'Both operations should be reverted when the first operation fails',
      );
    });
  });
}
